...
async function fetchData() {
    try {
        ...
        // 스키마 순서 원칙 강제 적용: A분류 / B아이콘 / C이름 / D패치 / E획득처 / F장게거래여부
        rawData = rows.slice(1).map((row) => {
            ...
            return {
                ...
                sub: '전체 목록',       
                icon: getVal(1),        // B열: 파싱된 이미지 파일 고정 주소
                name: itemName,         // C열: 이름
...
// 🌟 [전면 버그 수정 수술] 거래 가능/불가 클릭 시 타 필터 클래스 및 변수를 완전 무조건 파괴 및 초기화
function selectRewardFilter(type, btn) {
    ...
    if (type === 'ALL') {
        ...
        // ⚡ 1. 보유 상태 필터 변수를 무조건 초기화하고 HTML 버튼의 active 불빛도 강제 이동시킵니다.
        ...
        // ⚡ 2. 검색창 내부에 타이핑된 내용과 연동 변수를 강제로 공백 제거 청소합니다.
        currentSearchQuery = '';
        const searchInput = document.getElementById('search-keyword');
        if (searchInput) searchInput.value = '';

        // ⚡ 3. 활성화되어 있던 카테고리(A열) 버튼의 테두리 하이라이트 불빛을 완전 소등합니다.
        document.querySelectorAll('#main-category-group button').forEach(b => b.classList.remove('active'));
    ...
    renderList(); // 변수들이 완전 소멸 리셋된 도화지 상태에서 리스트를 전량 리렌더링
}
...
function renderList() {
    ...
    if (!currentSearchQuery) {
        if (currentRewardFilter === 'ALL') {
            ...
            // 🌟 카테고리 해제가 반영되어 전체 도감 레코드 중 거래유형 일치 대상을 통째로 추출
    ...
    filtered.forEach((item, idx) => {
        ...
        // 🌟 [이미지 미출력 전면 패치] 물음표 등 인코딩 충돌 없이 다이렉트 프록시 바이패스 설계로 전면 교체
        let finalIconUrl = '';
        if (item.icon) {
            // 주소 텍스트의 특수 요소를 공백 및 하위 프로토콜 규격으로 원복하여 프록시에 직접 인젝션합니다.
            const cleanUrlStr = item.icon.replace(/^(https?:\/\/)?/i, '');
            finalIconUrl = `https://weserv.nl{encodeURIComponent(cleanUrlStr)}&nocache=1`;
        }
